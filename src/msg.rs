...
/// Строки для отладочного окна.
///
/// Формат: `# ` - заголовок секции, два пробела - строка под ним. Разбирает
/// это `overlay::draw_debug`.
///
/// Здесь живёт ТОЛЬКО то, что сейчас выясняется: подтверждённое уезжает
/// отсюда сразу, иначе окно превращается в ленту, в которой не видно текущего
/// вопроса.
