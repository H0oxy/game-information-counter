...
pub enum SystemInitError {
    ...
    InvalidRva,
    /// The running game build isn't one this package has addresses for. Not a
    /// failure to wait - there is nothing here to wait on.
...
pub fn wait_for_system_init(module: &Program, timeout: Duration) -> Result<(), SystemInitError> {
    if std::ptr::eq(GLOBAL_HINSTANCE.load(Ordering::Relaxed), 0x0 as _) {
        let rvas = rva::try_get().ok_or(SystemInitError::UnsupportedVersion)?;
        let va = module
